...
const Main = () => {
  ...
  useEffect(() => {
    ...
    const fetchInitialMessages = async () => {
      try {
        ...
        setMessages(data);
        const initialRead = {};
        data.forEach((m) => { if (m.id) initialRead[m.id] = true; });
  ...
  const loadStatuses = useCallback(async () => {
    ...
    } catch (err) { console.error("Error fetching follow statuses:", err); }
  }, [users, token]);
  ...
  const sendFollowRequest = useCallback(async (receiverId) => {
    try {
      ...
      setFollowStatuses((prev) => ({ ...prev, [receiverId]: res.data.followStatus }));
    } catch (err) { console.error("Error sending follow request:", err); }
  }, [username, token]);

  const unfollowUser = useCallback(async (receiverId) => {
    try {
  ...
  const acceptRequest = useCallback(async (senderId) => {
    try {
      ...
      setPendingRequests((prev) => prev.filter((p) => p.senderId !== senderId));
      setFollowStatuses((prev) => ({ ...prev, [senderId]: "ACCEPTED" }));
      loadStatuses();
    } catch (err) { console.error("Accept error:", err); }
  }, [token, loadStatuses]);

  const rejectRequest = useCallback(async (senderId) => {
    try {
      await axios.post(
  ...
  useEffect(() => { if (activeTab === "nearby") fetchNearby(); }, [activeTab, fetchNearby]);

  /* ---------------------- CHAT HANDLERS ---------------------- */
  const handleMessages = useCallback(() => {
    if (!message.trim() || !user?.email || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
    const outgoing = { receiver: user.email, content: message.trim() };
    wsRef.current.send(JSON.stringify(outgoing));
    setMessages((prev) => [...prev, { ...outgoing, sender: email, timestamp: new Date().toISOString(), delivered: false, local: true }]);
    setMessage("");
  }, [message, user, email]);

  const deleteMessageForMe = useCallback(async (msg) => {
    setMessages((prev) => prev.filter((m) => (msg.id ? m.id !== msg.id : m !== msg)));
    if (!msg.id || !token) return;
    try { await axios.delete(`http://localhost:8081/api/messages/${msg.id}/me`, { headers: { Authorization: `Bearer ${token}` } }); }
    catch (err) { console.error("Error deleting message for me:", err); }
  }, [token]);

  const deleteMessageForBoth = useCallback(async (msg) => {
    setMessages((prev) => prev.filter((m) => (msg.id ? m.id !== msg.id : m !== msg)));
    if (!msg.id || !token) return;
    try { await axios.delete(`http://localhost:8081/api/messages/${msg.id}/both`, { headers: { Authorization: `Bearer ${token}` } }); }
    catch (err) { console.error("Error deleting message for both:", err); }
  }, [token]);

  const handleBubbleClick = useCallback((msg) => {
    const now = Date.now();
  ...
  useEffect(() => {
    ...
    setReadMessageIds((prev) => {
      const updated = { ...prev };
      messages.forEach((m) => { if (m.sender === user.email && m.receiver === email && m.id) updated[m.id] = true; });
      return updated;
    });
  }, [user, messages, email]);
  ...
  const unreadCountsByEmail = useMemo(() => {
    const counts = {};
    messages.forEach((m) => { if (m.receiver === email && m.id && !readMessageIds[m.id] && m.sender) counts[m.sender] = (counts[m.sender] || 0) + 1; });
    return counts;
  ...
  const displayedUsers = useMemo(() => {
    ...
    if (activeTab === "friends") return users.filter((u) => followStatuses[u.id] === "ACCEPTED");
    if (activeTab === "suggestions") return users.filter((u) => u.id !== userId);
  ...
  const conversationMessages = useMemo(() => {
    ...
    return messages.filter(
      (m) => (m.sender === email && m.receiver === user.email) || (m.sender === user.email && m.receiver === email)
    );
  }, [messages, user, email]);
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row overflow-hidden">
      <div className={`w-full lg:w-[30%] flex flex-col h-full border-r border-gray-200 ${user ? "hidden lg:flex" : "flex"}`}>
        ...
        <div className="flex flex-row gap-1 m-2 overflow-x-auto no-scrollbar">
          {["friends", "nearby", "suggestions", "requests"].map((tab) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); if (tab === "requests") fetchPending(); if (tab === "nearby") fetchNearby(); }}
              className={`flex-shrink-0 py-1 rounded-xl border border-white/20 px-2 text-xs md:text-sm ${activeTab === tab ? "bg-blue-500 text-white" : "bg-white/10 text-black"}`}
            >
        ...
        <div className="flex flex-col flex-1 overflow-y-auto no-scrollbar">
          {activeTab !== "requests" &&
            displayedUsers.filter((u) => u.username.toLowerCase().startsWith(search.toLowerCase())).map((u) => {
              const friendEmail = u.email;
              const unreadCount = friendEmail ? unreadCountsByEmail[friendEmail] || 0 : 0;
              return (
                <UserRow
                  key={u.id}
                  u={u}
          ...
          {activeTab === "requests" &&
            pendingRequests.map((req) => (
              <RequestRow key={req.senderId} req={req} acceptRequest={acceptRequest} rejectRequest={rejectRequest} />
            ))}
        </div>
      </div>

      <div className={`flex flex-col flex-1 w-full h-full bg-gray-100 ${user ? "flex" : "hidden lg:flex"}`}>
        <div className="flex items-center p-2 border-b md:hidden">
          {user && <button onClick={() => setUser(null)} className="px-2 py-1 bg-gray-200 rounded text-xs">Back</button>}
          <h1 className="font-bold text-sm ml-2">{user ? user.username : "Chat"}</h1>
        </div>
        <div className="hidden md:flex items-center p-3 border-b">
          <h1 className="font-bold text-xl md:text-2xl">{user ? user.username : "Select a friend to start chatting"}</h1>
        </div>

        <div ref={chatContainerRef} className="flex flex-col flex-1 overflow-y-auto p-2 space-y-1 no-scrollbar">
          {!user ? (
            <div className="w-full h-full flex items-center justify-center text-gray-500 text-sm md:text-base">👈 Select a friend from the list to start chatting</div>
          ) : (
            conversationMessages.map((msg, idx) => {
              ...
              return (
                <React.Fragment key={msg.id || idx}>
                  {showUnreadSeparator && <div className="w-full flex justify-center my-2"><span className="px-3 py-1 text-[10px] md:text-xs bg-green-100 text-green-700 rounded-full">Unread messages</span></div>}
                  <div
                    onClick={() => handleBubbleClick(msg)}
                    onTouchStart={handleTouchStart}
                    onTouchMove={handleTouchMove}
                    onTouchEnd={(e) => handleTouchEnd(e, msg)}
                    className={`p-1 border rounded w-fit max-w-[75%] text-xs md:text-base touch-pan-y ${isMe ? "bg-blue-500 text-white self-end" : "bg-white text-black self-start"}`}
                  >
                    {msg.content}
                  </div>
                </React.Fragment>
        ...
        {user && (
          <div className="flex items-center gap-2 p-2 border-t">
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleMessages(); } }}
              className="flex-1 h-10 md:h-11 px-3 border rounded text-xs md:text-sm"
              type="text"
              placeholder="Message"
            />
            <button onClick={handleMessages} className="h-10 md:h-11 px-4 rounded text-xs md:text-sm bg-blue-500 text-white flex items-center justify-center">Send</button>
          </div>
        )}
      </div>
    </div>
